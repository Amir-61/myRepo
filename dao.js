...
//**sonali**
function byWhereQuery(Model, exp) {

  var keys = Object.keys(where);
  if (keys.length != 1) return false;
  var element = Object.keys(exp)[0];
  var query = {
    where: {}
  };
  var pk = idName(Model);
  ...
  query.where[element] = exp[element];
  return query;
...
/**
 * Performs upsert based on the instance that matches the where clause
...
DataAccessObject.upsertWithWhere = function(where, data, options, cb) {
  ...
  assert(typeof connector.update === 'function',  'upsertWithWhere() must be implemented by the connector');
  var query = {
    where: where
  };
  this.applyScope(query);
  ...
  Model.notifyObserversOf('access', context, function(err, ctx) {
    ...
    Model.notifyObserversOf('before save', context,  function(err, ctx) {
      if (err) return cb(err);
      doUpsertWithWhereNPK(ctx.where, ctx.data);
    });
  });

  function doUpsertWithWhereNPK(err, ctx) {
    if (err) return cb(err);
    if (connector.updateOrCreate) {
      var context = {
        Model: Model,
        where: ctx.query.where,
        data: data,
      ...
      Model.notifyObserversOf('before save', context, function(err, ctx) {
          ...
          inst.isValid(function(valid) {
            if (!valid) {
              if (doValidate) { // backwards compatibility with validateUpsert:undefined
                ...
                return cb(new ValidationError(inst), inst);
              } else {  // TODO(bajtos) Remove validateUpsert:undefined in v3.0

                console.warn('Ignoring validation errors in updateOrCreate():');
                console.warn('  %s', new ValidationError(inst).message);  // continue with updateOrCreate
        ...
        function callConnector() {
          ...
          Model.notifyObserversOf('persist', context, function(err) {
            if (err) return done(err);
            if (connector.upsertWithWhere.length === 5) {
              connector.upsertWithWhere(Model.modelName, where, update, options, done);
            } else {
              connector.upsertWithWhere(Model.modelName, where, update, done);
            }
          });
        }
      ...
      Model.findOne({
        ...
        if (!isOriginalQuery) {  // The custom query returned from a hook may hide the fact that
          // there is already a model with `id` value `data[idName(Model)]`
